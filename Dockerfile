...
CMD ["sh", "-lc", "echo '[Gunicorn] Starting on port: ${PORT:-8080}' && exec gunicorn --workers ${WEB_CONCURRENCY:-3} --bind 0.0.0.0:${PORT:-8080} --log-level debug --access-logfile - --error-logfile - app:app"]